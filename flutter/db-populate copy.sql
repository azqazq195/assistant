...
CREATE TABLE IF NOT EXISTS `csttec`.`cobject` (
  ...
  `copenness` TINYINT NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `csttec`.`citem_revision` (
  ...
  `ctime_required` DOUBLE NULL COMMENT '소요시간',
...
CREATE TABLE IF NOT EXISTS `csttec`.`ccontact` (
  ...
  `ctype` VARCHAR(20) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_ccontact_place_idx` (`place_id` ASC),
  INDEX `fk_ccontact_object_idx` (`object_id` ASC),
  CONSTRAINT `fk_ccontact_place`
    FOREIGN KEY (`place_id`)
    REFERENCES `csttec`.`cplace` (`id`)
    ON DELETE CASCADE
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_ccontact_object`
    FOREIGN KEY (`object_id`)
    REFERENCES `csttec`.`cobject` (`id`)
    ON DELETE CASCADE
    ON UPDATE RESTRICT)
...
CREATE TABLE IF NOT EXISTS `csttec`.`chz_rfq` (
  ...
  CONSTRAINT `fk_chz_rfq_customer`
    ...
    REFERENCES `csttec`.`ccustomer` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
...
CREATE TABLE IF NOT EXISTS `csttec`.`cproject_evm` (
  ...
  `cev_md` INT NULL,
  `cac_md` INT NULL,
  INDEX `fk_cproject_evm_cproject1_idx` (`project_id` ASC),
  PRIMARY KEY (`project_id`, `cdate`),
  CONSTRAINT `fk_cproject_evm_cproject1`
    FOREIGN KEY (`project_id`)
    REFERENCES `csttec`.`cproject` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Table `csttec`.`cobject_characteristic`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `csttec`.`cobject_characteristic` ;

CREATE TABLE IF NOT EXISTS `csttec`.`cobject_characteristic` (
  `object_id` INT NOT NULL,
  `characteristic_id` INT NOT NULL,
  INDEX `fk_cobject_characteristic_object_idx` (`object_id` ASC),
  INDEX `fk_cobject_characteristic_characteristic_idx` (`characteristic_id` ASC),
  CONSTRAINT `fk_cobject_characteristic_object`
    FOREIGN KEY (`object_id`)
    REFERENCES `csttec`.`cobject` (`id`)
    ON DELETE CASCADE
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cobject_characteristic_characteristic`
...
-- -----------------------------------------------------
-- Table `csttec`.`cop_step`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `csttec`.`cop_step` ;

CREATE TABLE IF NOT EXISTS `csttec`.`cop_step` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `cname` VARCHAR(45) NULL,
  `cdescription` VARCHAR(500) NULL,
  `coperation_type` TINYINT NULL,
  `chas_outflow` TINYINT(1) NULL,
  `rework_to_op_step_id` INT NULL,
  `rework_out_op_step_id` INT NULL,
  `co_user_id` INT NULL,
  `user_id` INT NOT NULL,
  `cdate_modified` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_cop_step_rework_to_idx` (`rework_to_op_step_id` ASC),
  INDEX `fk_cop_step_rework_out_idx` (`rework_out_op_step_id` ASC),
  INDEX `fk_cop_step_co_user_idx` (`co_user_id` ASC),
  INDEX `fk_cop_step_user_idx` (`user_id` ASC),
  CONSTRAINT `fk_cop_step_rework_to`
    FOREIGN KEY (`rework_to_op_step_id`)
    REFERENCES `csttec`.`cop_step` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cop_step_rework_out`
    FOREIGN KEY (`rework_out_op_step_id`)
    REFERENCES `csttec`.`cop_step` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cop_step_co_user`
    FOREIGN KEY (`co_user_id`)
    REFERENCES `csttec`.`cuser` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cop_step_user`
    FOREIGN KEY (`user_id`)
    REFERENCES `csttec`.`cuser` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT)
...
CREATE TABLE IF NOT EXISTS `csttec`.`ccontrol_plan` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `cmeasure_method` VARCHAR(500) NULL,
  `csample_size` VARCHAR(30) NULL,
  `csample_frequency` VARCHAR(30) NULL,
  `ccontrol_method` VARCHAR(100) NULL,
  `ccontrol_responsibility` TINYINT NULL,
  `creaction_plan` VARCHAR(500) NULL,
  `co_user_id` INT NULL,
  `user_id` INT NOT NULL,
  `cdate_modified` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_ccontrol_plan_co_user_idx` (`co_user_id` ASC),
  INDEX `fk_ccontrol_plan_user_idx` (`user_id` ASC),
  CONSTRAINT `fk_ccontrol_plan_co_user`
    FOREIGN KEY (`co_user_id`)
    REFERENCES `csttec`.`cuser` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_ccontrol_plan_user`
    FOREIGN KEY (`user_id`)
    REFERENCES `csttec`.`cuser` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT)
...
-- -----------------------------------------------------
-- Table `csttec`.`cpfd_data`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `csttec`.`cpfd_data` ;

CREATE TABLE IF NOT EXISTS `csttec`.`cpfd_data` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `thread_id` INT NULL,
  `doc_id` INT NOT NULL,
  `op_rev_id` INT NOT NULL,
  `op_step_id` INT NULL,
  `cop_step_seq` TINYINT NULL,
  `characteristic_id` INT NULL,
  `ccharacteristic_category` TINYINT NULL,
  `control_plan_id` INT NULL,
  INDEX `fk_cpfd_data_op_step_idx` (`op_step_id` ASC),
  INDEX `fk_cpfd_data_doc_idx` (`doc_id` ASC),
  INDEX `fk_cpfd_data_op_rev_idx` (`op_rev_id` ASC),
  INDEX `fk_cpfd_data_characteristic_idx` (`characteristic_id` ASC),
  INDEX `fk_cpfd_data_control_plan_idx` (`control_plan_id` ASC),
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_cpfd_data_op_step`
    FOREIGN KEY (`op_step_id`)
    REFERENCES `csttec`.`cop_step` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cpfd_data_doc`
    FOREIGN KEY (`doc_id`)
    REFERENCES `csttec`.`cdoc` (`id`)
    ON DELETE CASCADE
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cpfd_data_op_rev`
    FOREIGN KEY (`op_rev_id`)
    REFERENCES `csttec`.`citem_revision` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cpfd_data_characteristic`
    FOREIGN KEY (`characteristic_id`)
    REFERENCES `csttec`.`ccharacteristic` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cpfd_data_control_plan`
    FOREIGN KEY (`control_plan_id`)
    REFERENCES `csttec`.`ccontrol_plan` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `csttec`.`cop_step_ignored`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `csttec`.`cop_step_ignored` ;

CREATE TABLE IF NOT EXISTS `csttec`.`cop_step_ignored` (
  `object_id` INT NOT NULL,
  `op_step_id` INT NULL,
  INDEX `fk_cop_step_ignored_object_idx` (`object_id` ASC),
  INDEX `fk_cop_step_ignored_op_idx` (`op_step_id` ASC),
  CONSTRAINT `fk_cop_step_ignored_object`
    FOREIGN KEY (`object_id`)
    REFERENCES `csttec`.`cobject` (`id`)
    ON DELETE CASCADE
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_cop_step_ignored_op_step`
    FOREIGN KEY (`op_step_id`)
    REFERENCES `csttec`.`cop_step` (`id`)
    ON DELETE CASCADE
    ON UPDATE RESTRICT)
...
insert into cdept(cname, cdate_created, cactive)
values('조직도', now(), 1);
...
INSERT INTO cwork_type (thread_id, thread_current, user_id, cname, cdescription, cstereotype, cactive) 
VALUES (2, 1, 0, '마일스톤', '프로젝트의 중요 시점을 의미하며 통과기준 등을 검토하는 업무입니니다.\n시스템의 정상적 동작을 위해 필요한 정보이므로 삭제 또는 수정하지 마십시오.', 'milestone', 1);
